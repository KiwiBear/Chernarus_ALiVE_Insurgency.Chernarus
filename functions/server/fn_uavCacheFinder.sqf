...
if (_numMarkers > 0) then {
	_Message = format["HQ: HQ to all units, UAV drone operators have updated your maps with %1 possible cache locations. HQ out.", _numMarkers];
	_Message remoteExec ["systemchat",west];
}else{
	_Message = "HQ: HQ to all units, UAV drone operators were unable to find any possible cache locations. HQ out.";
	_Message remoteExec ["systemchat",west];
};
